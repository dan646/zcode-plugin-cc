...
argument-hint: '[base-branch|base-commit|path] [--model <id>] [--cwd <path>] [--timeout <seconds>] [--mode <build|edit|plan|yolo>] [--quiet] [--no-stream]'
...
Progress (state changes, and one line per tool call ZCode makes with relative paths, e.g. `[zcode] tool: Bash git log -p`) and streaming model text go to stderr as the turn runs; pass `--no-stream` to suppress streaming model text while keeping tool calls and heartbeats, or `--quiet` to suppress all progress and print only the final review and usage summary.
