...
argument-hint: '[base-branch|base-commit|path] [--model <id>] [--cwd <path>] [--timeout <seconds>] [--idle-after-write <seconds>] [--max-tool-calls <n>] [--mode <build|edit|plan|yolo>] [--quiet] [--no-stream]'
...
Progress (state changes, and one line per tool call ZCode makes with relative paths, e.g. `[zcode] tool: Bash git log -p`) and streaming model text go to stderr as the turn runs. On a stopped turn, stdout instead includes the buffered tail of `text_delta` marked as an unfinished response, never as a final review. Pass `--no-stream` to suppress streaming model text while keeping tool calls and heartbeats, or `--quiet` to suppress all progress and print only the final review and usage summary.
