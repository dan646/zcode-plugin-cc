...
| `edit` | **1 — только на `Bash`**; `Write` одобряется без запроса | да |
| `plan` | 0 | нет — режим ничего не пишет |
| `yolo` | **0** | да |
...
- Любое ограничение области правок (`--allow`/`--deny`) реализуемо **только** в режиме
  `build`: в `yolo` ZCode не спрашивает вообще, а в `edit` сам одобряет `Write` и спрашивает
  только про `Bash`. Сочетание области с `edit` или `yolo` надо отвергать, а при заданной
  области **явно** выставлять `build` — режим по умолчанию может быть переопределён в
  конфигурации ZCode, и тогда область молча перестанет работать.
- Файловая система macOS по умолчанию нечувствительна к регистру, а `fs.realpathSync` регистр
  к фактическому не приводит: `Resources/JS/x.ts` и `resources/js/x.ts` — один файл, но разные
  строки. Сопоставлять пути с шаблонами области на `darwin` нужно без учёта регистра.
